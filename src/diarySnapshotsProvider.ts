...
export class DiarySnapshotsProvider implements vscode.TreeDataProvider<DiaryItem | SnapshotItem> {
    ...
    getChildren(element?: DiaryItem | SnapshotItem): Thenable<DiaryItem[] | SnapshotItem[]> {
        if (element instanceof DiaryItem) {
            ...
            var d: Diary | undefined = this.context.globalState.get("diary");
            if (d!== undefined && d.snapshots.length > 0) {
                return Promise.resolve(
                    this.getSnapshots(d.snapshots, d)
                );
            }
            else {
                return Promise.resolve(
